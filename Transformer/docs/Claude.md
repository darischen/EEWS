...
### Data Flow
```
Yahoo Finance (daily OHLCV)  →  Finnhub + Reddit (sentiment)  →  CSV/Database
                                                                    ↓
                                                        (Auto-fetch on train)
                                                                    ↓
                                                        Normalize + Sequences
...
User: python train.py
    ↓
Fetch latest data from Yahoo + Finnhub/Reddit (automatic)
    ↓
...
│   │   ├── data/
│   │   │   ├── __init__.py
│   │   │   ├── fetch.py               # Yahoo Finance + Finnhub/Reddit
...
│   │
│   ├── data/
│   │   ├── raw/                       # Downloaded from Yahoo/Finnhub/Reddit
│   │   │   └── .gitkeep
...
```
# Finnhub and Reddit sentiment APIs (replacing deprecated NewsAPI)
FINNHUB_API_KEY=your_finnhub_api_key_here
REDDIT_CLIENT_ID=your_reddit_client_id_here
REDDIT_CLIENT_SECRET=your_reddit_client_secret_here
REDDIT_USER_AGENT=eews-sentiment-bot/1.0
```

**Note**: NewsAPI integration has been deprecated in favor of Finnhub (institutional news) + Reddit (retail sentiment) for better historical coverage and multi-signal regime detection.
...
✅ **Sentiment scraping**: Finnhub (news) + Reddit (social) integration
...
4. Get Finnhub and Reddit API credentials (free tiers available)
